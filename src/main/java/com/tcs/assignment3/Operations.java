...
public class Operations {
	...
	private static void updateDeposit(Statement statement, int i) {
	...
	private static void update(Statement statement, int id, int amount) throws SQLException {
		...
		Account account = accounts.get(0); // Updating on the 1st account
		account.withdraw(100); // withdrawing hard coded value

	}

	private static void insert(Statement statement) throws SQLException {
		statement.execute(
				"INSERT INTO account(ID,owner_name,city,state,pin,phone_number,house_number,street,balance,status,account_type) VALUES(4,'Shahid','Mumbai', 'Maharashtra', 400054, 1234567890, 12, 'Holy Street', 50000, 'ACTIVE' , 'SAVINGS')\r\n"
						+ "");
